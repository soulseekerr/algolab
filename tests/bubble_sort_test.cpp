...
TEST_F(BubbleSortTest, SortReverseOrderedVector) {
...
TEST_F(BubbleSortTest, SortRandomNumbers) {
    algolab::bubbleSort<int>(randomNumbers_int);
    // Check that the vector is sorted correctly
    EXPECT_TRUE(isSorted(randomNumbers_int));

    algolab::bubbleSort<float>(randomNumbers_flt);
    EXPECT_TRUE(isSorted(randomNumbers_flt));
}

TEST_F(BubbleSortTest, /*DISABLED_*/BenchmarkSortRandom) {
    auto start = std::chrono::high_resolution_clock::now();
    algolab::bubbleSort(randomNumbers_int);
    auto end = std::chrono::high_resolution_clock::now();
    std::cout << "BubbleSort (int) time: " 
              << std::chrono::duration<double, std::milli>(end - start).count()
              << " ms\n";
    
    start = std::chrono::high_resolution_clock::now();
    algolab::bubbleSort(randomNumbers_flt);
    end = std::chrono::high_resolution_clock::now();
    std::cout << "BubbleSort (float) time: " 
              << std::chrono::duration<double, std::milli>(end - start).count()
              << " ms\n";
}
